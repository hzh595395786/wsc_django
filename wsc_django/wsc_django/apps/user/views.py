...
from user.serializers import UserCreateSerializer, SuperUserSerializer, EmailSerializer
...
class SuperUserView(SuperBaseView):
    ...
    def put(self, request, args):
        ...
        update_user_basic_data(user, args)
...
class AdminUserAuthorizationView(UserBaseView):
    """后台-用户登录认证"""
...
class SMSCodeView(GlobalBaseView):
    ...
    def post(self, request, args):
        ...
        #     use = "绑定手机号"
        #     # 先用腾讯发,失败就云片用发
        #     ret, info = TencentSms.send_tencent_verify_code(phone, int(sms_code), use)
        #     if not ret:
