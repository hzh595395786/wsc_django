...
urlpatterns_admin = [
    ...
    path('api/admin/user/authorization/', views.AdminUserAuthorizationView.as_view()), # 后台验证登录状态
    path('api/admin/user/sms_code/', views.SMSCodeView.as_view()),  # 后台发送短信验证码
]

urlpatterns_mall = [
    re_path(r'^api/mall/user/(?P<shop_code>\w+)/$', views.MallUserView.as_view()),  # 商城端用户注册登录
    path('api/mall/user/sms_code/', views.SMSCodeView.as_view()),  # 商城端短信验证码
]
